 ...
 */
openerp.pos_six = function(instance){
    ...
    var OrderModelParent = module.Order;
    module.Order = module.Order.extend({
        removePaymentline: function(line){
            ...
            if ((line.ref_number) && (line.ref_number > '')) {
                ...
                // Normal Super Call
                OrderModelParent.prototype.removePaymentline.apply(this, arguments);
            }
        },
        addPaymentline: function(cashregister) {
            // Do make super call
            OrderModelParent.prototype.addPaymentline.apply(this, arguments);
    ...
    var PaymentlineModelParent = module.Paymentline;
    module.Paymentline = module.Paymentline.extend({
        ...
        set_transaction_amount: function(value){
            this.transaction_amount = round_di(parseFloat(value) || 0, this.pos.currency.decimals);
    ...
    var MPD = instance.web.Class.extend({
        shift_state: 'unknown',
        closed_manual: false,
        ...
        payment_terminal_initialize: function() {
            ...
            this.pos.pos_widget.mpd_popup.show();
                ...
                function done(result) {
                    if (result.success==true) {
                        ...
                        if (order.selected_paymentline.cashregister.journal.auto_validate) {
                            self.pos.pos_widget.payment_screen.validate_order();
                        }
                    } else {
        ...
        open_shift: function(cashier) {
            this.pos.pos_widget.mpd_popup.loading(_t('Schicht wird geöffnet...'));
            ...
            this.proxy.message('mpd/open', {'cashier': cashier}, {'timeout': 15000}).then(
                function done(result) {
                    ...
                    self.pos.pos_widget.mpd_popup.loading_done();
                },
                function failed() {
                    console.log('open shift failed');
                    self.pos.pos_widget.screen_selector.show_popup('error',{
                        message: _t('Fehler bei der Anfrage an das Terminal'),
                    });
                    self.pos.pos_widget.mpd_popup.loading_done();
                }
            );
        ...
        close_shift: function(manual) {
            ...
            this.pos.pos_widget.mpd_popup.loading(_t('Schicht wird geschlossen...'));
        ...
        // Call open shift function - do return the deferred object
        balance: function() {
            this.pos.pos_widget.mpd_popup.loading(_t('Terminal Abrechnung wird übertragen. Dieser Vorgang kann etwas Zeit in Anspruch nehmen. Bitte um Geduld !'));
            ...
            this.proxy.message('mpd/balance', {}, {'timeout': 60000}).then(
                ...
                function failed() {
                    ...
                    self.pos.pos_widget.screen_selector.show_popup('error',{
                        message: _t('Fehler bei der Balance Operation'),
                    });
                    self.pos.pos_widget.mpd_popup.loading_done();
                    dfd.resolve();
                }
    ...
    module.MPDPopupWidget = module.PopUpWidget.extend({
    ...
    module.MPDRefWidget = module.PopUpWidget.extend({
    ...
    module.MPDStatusWidget = module.StatusWidget.extend({
        ...
        start: function(){
            ...
            this.$el.click(function(){
                ...
                self.pos_widget.mpd_popup.show({terminal: self.pos.get('mpdstatus'), cashier: cashier});
            });
        ...
        close: function() {
            ...
            if ((this.pos.config.auto_terminal_shift) && (this.pos.mpd.connected())) {
                ...
                this.pos.mpd.balance().then(function() {
                    PosWidgetSuper.prototype.close.apply(self, arguments);
                });
            } else {
                PosWidgetSuper.prototype.close.apply(self, arguments);
            }
    ...
    var ProxyDeviceSuper = module.ProxyDevice;
